...
updateStatus(id)
{
  Swal.fire({
    ...
    if (result.isConfirmed) {
      Axios.put(AppUrls._baseUrl + actionUrl._updateOrderById,data,{
      })
       .then(response => {        
         console.log(response)
         if (response.data.code == 200 && response.data.type == "success") {
...
openXl(content,selObj) {
  ...
  updateSataus(id,type)
  {
    Swal.fire({
      // title: 'Are you sure?',
      text: `Are you sure to ${type} this order ?`,
      icon:  type == 'CONFIRMED' ? 'success' : 'warning',
      showCancelButton: true,
      confirmButtonColor: '#3085d6',
      cancelButtonColor: '#d33',
      confirmButtonText: 'Yes',
        ...
        Axios.put(AppUrls._baseUrl + actionUrl._updateOrderStatus,data,{
         ...
         .then(response => {        
           ...
           if (response.data.code == 200 && response.data.type == "success") {
            ...
            //  this.getOrdeList();
            if(type == 'CONFIRMED')
            this.router.navigateByUrl("/confirmed-orders")
            else
            this.router.navigateByUrl("/cancelled-orders")
           }
