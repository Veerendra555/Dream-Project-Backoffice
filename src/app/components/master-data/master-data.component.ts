...
export class MasterDataComponent implements OnInit {
  ...
  constructor(  private  router: Router,
    ...
    public snackBar:MatSnackBar) {
      ...
      this.name = this.router.url == '/terms-and-conditions' ? "TermsAndConditions" : "PrivacyPolicy"
       this.master.type = this.name;
      this.getMasterByName();
     }
  

  formSubmit(form:NgForm)
  {
    Axios.post(AppUrls._baseUrl + actionUrl._addMasters,this.master,{
      headers: {
  ...
  changeStatus()
  {
    this.masterStatus=!this.masterStatus;
  }

  getMasterByName(){
    Axios.get(AppUrls._baseUrl + actionUrl._getDetailsByName+this.name,{
      // headers: {
...
async selectFile(event) {
  ...
  upload() {  // (file)
    ...
    self.uploadService.uploadFileAny(file ,function (err ,key) {
      ...
      if(self.id != null)
      {
       self.uploadService.deleteImage(self.master.imageURL);
       self.master.imageURL=self.imageUrl+"masters/"+self.keyPath;
      }
    //   else {
    //      console.log(key);
