...
export class AddCategoriesComponent implements OnInit {
  ...
  {
    // this.categorie.categorieImageUrl= "https://images.unsplash.com/photo-1628102491629-778571d893a3?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=580&q=80"; 
     delete this.categorie.imageUrl;
    if (this.id == null) {
      this.categorie.imageURL=this.imageUrl+"categories/"+this.keyPath;
    Axios.post(AppUrls._baseUrl + actionUrl._addCategories,this.categorie,{
      headers: {
      ...
      .then(response => {        
        ...
        if (response.data.code == 201 && response.data.type == "success") 
        {
          ...
          this.router.navigateByUrl('view-categorie');
          this.backClicked()
        }
        else
        {
  ...
  delete this.categorie.isActive; 
  delete this.categorie.__v;
  delete this.categorie.displayStatus;
  Axios.put(
    AppUrls._baseUrl + actionUrl._updateCategorie,this.categorie,
    // {
    //   headers:{
...
async selectFile(event) {
  ...
  upload() {  // (file)
    ...
    self.uploadService.uploadFileAny(file ,function (err ,key) {
      ...
      if(self.id != null)
      {
       self.uploadService.deleteImage(self.categorie.imageURL);
       self.categorie.imageURL=self.imageUrl+"categories/"+self.keyPath;
      }
    //   else {
    //      console.log(key);
